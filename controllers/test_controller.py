...
# Создаем пустой граф для хранения топологии сети
topo = nx.DiGraph()
...
class DijkstraController(EventMixin):
    def __init__(self):
        ...
        self.mac_to_port = {}
        self.port_stats = {}
        self.update_interval = 5  # Интервал обновления статистики в секундах
        self.bandwidth = 10  # Начальная пропускная способность каждого канала
...
def launch():
    ...
    discovery_launch(link_timeout=15, eat_early_packets=True)  # Уменьшен таймаут для ссылок
    stp_launch()  # Запуск протокола STP
